...
public class RentACat {
    ...
    public boolean returnCat(Cat c) {
		if (c.getRented()) {
			// If the cat has been rented, then return the cat
			c.returnCat();
			return true;
		}
		else {
			// If the cat has not been rented, then it cannot be returned
			return false;
		}
    }
    ...
    public boolean rentCat(Cat c) {
		if (c.getRented()) {
			// If the cat has already been rented, then it cannot be rented
			return false;
		}
		else {
			// If the cat has not been rented, then rent the cat
			c.rentCat();
			return true;
		}
    }
    ...
    public String listCats(ArrayList<Cat> catList) {
		String newCatList = "";
		for (Cat c: catList) {
			// Check that the cat has not already been rented.
			// If the cat is not rented, then add it to the String.
			if (c.getRented() == false) {
				newCatList = newCatList + (c.toString() + "\n");
			}
		}
		return newCatList;
    }
    ...
    public boolean catExists(int id, ArrayList<Cat> catList) {
		...
		if (catList == null || catList.size() == 0) {
			...
			if (catList.contains(id)) {
				// If the correct id is in the list, then the cat is found
				isFound = true;
			}
			else {
				// Otherwise, the cat is not found
				isFound = false;
			}
		}
		return isFound;
